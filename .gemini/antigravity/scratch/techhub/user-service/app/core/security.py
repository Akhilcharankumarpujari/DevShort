...
def create_access_token(subject: Union[str, Any], role: str = "customer", expires_delta: Optional[timedelta] = None) -> str:
    """
    Create a JWT access token containing subject (user_id), role, and custom JWT id (jti).
    """
    if expires_delta:
...
def create_refresh_token(subject: Union[str, Any], expires_delta: Optional[timedelta] = None) -> str:
    """
    Create a JWT refresh token containing subject (user_id) and custom JWT id (jti).
    """
    if expires_delta:
        expire = datetime.now(timezone.utc) + expires_delta
    else:
